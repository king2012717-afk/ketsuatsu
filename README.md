...
- 測定日時、時間帯（朝／晩／その他・時刻から自動判定）、測定した腕、服薬の有無、メモ
- 撮影した写真も記録に添付（設定でオフ可）
- 日本高血圧学会ガイドライン（JSH2019）に基づく判定を入力中にリアルタイム表示

### ⏰ リマインダー
- 時刻・曜日・時間帯を指定した複数のリマインダー
- 毎日の場合は 1 件の繰り返し通知、曜日指定なら曜日ごとに登録
- 通知の許可状態を画面上で確認・誘導
...
### 📊 グラフ・集計
- 期間切り替え（1週間 / 1か月 / 3か月 / 1年 / すべて）
- 血圧の推移（日ごとの平均 or 個別の測定値）＋目標値ライン
- 脈拍の推移
- 平均・最高・最低・測定回数・目標達成率・高血圧域の割合
- 判定区分ごとの内訳
- 朝と晩の平均比較、朝晩差（モーニングサージの目安）
- 先週比のトレンド表示
