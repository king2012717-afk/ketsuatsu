...
- アイコン画像は `AppMark` として画面内でも使用（ホームの空状態・設定画面）

### アプリアイコンの外観

iOS 18 以降はホーム画面の外観（通常・ダーク・色合い）ごとにアイコンを持てます。指定がないとシステムが元の絵から自動生成してしまうため、3 つとも用意しています。

| 外観 | ファイル | 内容 |
| --- | --- | --- |
| 通常 | `AppIcon.png` | コーラルのグラデーション＋白い血圧計 |
| ダーク | `AppIcon-Dark.png` | 通常とまったく同じ絵（暗くしない） |
| 色合い | `AppIcon-Tinted.png` | 背景を暗くしたグレースケール |

ダークモードでも見た目を変えたくないため、ダーク用は通常と同一の画像を明示的に指定しています。「色合いを付ける」を選んだ場合だけは、iOS が必ずユーザーの選んだ色を乗せる仕様のため、単色になります。

アイコンを差し替える場合は上記 3 ファイルと `AppMark.imageset/AppMark.png`（角丸・透過。アプリ内で使用）を置き換え、`Theme.swift` の 3 色を新しいアイコンから採色し直してください。
